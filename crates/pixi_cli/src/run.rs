...
use pixi_core::{
    ...
    environment::{LockFileUsage, PlatformData, sanity_check_workspace},
...
pub struct Args {
    ...
    /// Run a Python script using its inline PEP 723 metadata.
    #[arg(
        long,
        value_name = "PATH",
        conflicts_with_all = [
...
mod tests {
    ...
    #[test]
    fn script_mode_is_explicit_and_forwards_arguments() {
        let args =
            Args::try_parse_from(["run", "--script", "example.py", "first", "--second"]).unwrap();
        assert_eq!(args.script, Some(PathBuf::from("example.py")));
        assert_eq!(args.task, ["first", "--second"]);

        let args = Args::try_parse_from(["run", "example.py"]).unwrap();
        assert!(args.script.is_none());
        assert_eq!(args.task, ["example.py"]);
    }

    #[test]
    fn script_mode_rejects_workspace_and_environment_selection() {
        assert!(
            Args::try_parse_from([
                "run",
                "--script",
        ...
        assert!(
            Args::try_parse_from(["run", "--script", "example.py", "--environment", "dev",])
                .is_err()
        );
    }
